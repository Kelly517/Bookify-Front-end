...
const handleChapterSubmit = async (chapterTitle) => {
  ...
  console.log("Este es el título del capítulo: ", chapterTitle);

  try {
    await axios.post(`http://localhost:8080/api/bookify/page/${createdBookId}`, {
    ...
    localStorage.setItem("pageTitle", chapterTitle);
    localStorage.setItem("bookId", bookId);
    localStorage.setItem("bookIdentifierCode", bookIdentifierCode);
    setShowChaperModal(false);
    onClose();
    navigate(`/dashboard/write/create-page/${createdBookId}`, { replace: true });
  } catch(error) {
